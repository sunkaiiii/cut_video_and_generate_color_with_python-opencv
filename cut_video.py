...
def frame_to_time(frame,rate):
    ...
    """
    arr=[0,0,0,0]
    arr[3]=int((1000/rate)*(frame%rate)) #计算毫秒
...
def write_log(startFrame,stopFrame,rate,filepath,filename,cut_filename):
    ...
    durationTime = str(durationList[0]) + ':' + str(durationList[1]) + ':' + str(durationList[2]) + ',' + str(
        durationList[3])
    cut_info = str(
        cut_filename + '\t' + str(startFrame) + '\t' + str(stopFrame) + '\t' + startTime + '\t' + stopTime +'\t'+ durationTime+'\n')
    fp.write(cut_info)
    fp.close()
...
def save_video(frames,frameToStart,frameToStop,rate,saveNums,filename,size):
    ...
    filepath=os.getcwd()+'\\middle_file\\'+filename.strip('.'+filename.split('.')[len(filename.split('.'))-1])#路径指定视频中间文件的路径为middle_file文件夹下原视频文件名
    ...
    subprocess.call('C:\\ffmpeg\\bin\\ffmpeg.exe -y -i '+filepath+'\\'+filename+' -i '+filepath+'\\'+filename+'.mp3'+'  -vcodec copy -acodec copy '+savePath+'\\'+filename)
...
def save_video_with_ffmpeg(filename,startFrame=0,stopFrame=1,rate=24,saveNumber=0,mode=0,kbps=2048):
    ...
    if(stopFrame<startFrame):
        ...
        return False
    filename = filename.split('\\')[len(filename.split('\\')) - 1]  # 去除文件路径，保留文件名
    filepath=os.getcwd()+'\\cut\\'+filename.strip('.'+filename.split('.')[len(filename.split('.'))-1])#路径指定为cut文件夹下原视频文件名
    if not os.path.exists(filepath):
        os.mkdir(filepath)
    '''
    ...
    具体参考官方文档
    '''
    cut_filename=filename.strip('.'+filename.split('.')[len(filename.split('.'))-1])+'_'+str(saveNumber)+'.'+filename.split('.')[len(filename.split('.'))-1] #将文件名转换为原文件名+计数序号+扩展名
    print(rate)
    if mode==0:
        subprocess.call('C:\\ffmpeg\\bin\\ffmpeg.exe -y -ss '
                        + str(startFrame / rate) + ' -t ' + str((stopFrame - startFrame) / rate)
                        + ' -i ' + cut_file_name
                        + '  -c:v libx264 -preset superfast -b:v '+str(kbps)+'k -maxrate '+(str(kbps/2))+'k -bufsize '+str(kbps)+'k -c:a copy '
...
def compress_video(filename,compression_type='mpeg4',kpbs=2048,gop=2):
    ...
    middle_file=os.getcwd()+'\\middle_file\\'+filename.split('\\')[len(filename.split('\\')) - 1]
    print('C:\\ffmpeg\\bin\\ffmpeg.exe -y -i ' + filename + ' -ss ' + '0'+ ' -t ' + str(totalFrameNumber/rate) + ' -ab 128k -ar 44100 -b:v 2048k -r '+middle_file)
...
cut_video('E:\\test.mp4',mode=3,boundary=12)
